...
func TestGeneratePasswdPBKDF2Key(t *testing.T) {
	...
	for _, testV := range testCases {
		passwd, _ := encryption.DefaultPBKDF2Options.GeneratePasswdPBKDF2Key([]byte(testV.rawtext), []byte(testV.saltStr))
...
func TestCheckPBKDF2PasswdForVzhuoTaskSYS(t *testing.T) {
	...
	for _, testV := range testCases {
		checkReslt := encryption.DefaultPBKDF2Options.CheckPBKDF2PasswdForVzhuoTaskSYS(testV.rawtext, testV.passwd, testV.saltStr)
		if checkReslt != true {
			t.Errorf("验证[%s]失败\n", testV.rawtext)
		}
	}
